...
def display_confusion_matrix(model, y_test, y_pred, le, plots_dir):
    ...
    plt.savefig(
        os.path.join(plots_dir, f"{model}_confusionmatrix.pdf"), bbox_inches="tight", dpi=300)
    # plt.close()
    plt.show()
    return cm
...
def sensitivity_specificity_per_class(model, cm, le, plots_dir):
    ...
    metrics_df.to_csv(os.path.join(plots_dir, f"{model}_sensitivityspecificityresults.csv"), index=False)
    print("\nPer-class Sensitivity & Specificity:\n")
...
def display_roc_curves(model, y_test, y_prob, plots_dir, le):
    ...
    plt.savefig(
        os.path.join(plots_dir, f"{model}_ROC_allclasses.pdf"),
        bbox_inches="tight",
        dpi=300
    )
    ...
    for i in range(len(classes_idx)):
        ...
        plt.savefig(
            os.path.join(plots_dir, f"{model}_ROC_{le.classes_[i]}.pdf"),
            bbox_inches="tight",
            dpi=300
        )
    ...
    plt.savefig(
        os.path.join(plots_dir, f"{model}_ROC_macro_only.pdf"),
        bbox_inches="tight",
        dpi=300
    )
...
def shap_plot(model, final_model, X_train_selected, clean_features, le, plots_dir):
    ...
    for i, class_name in enumerate(class_labels):
        ...
        shap.summary_plot(
            ...
            show=False)
        plt.savefig(
            os.path.join(plots_dir, f"{model}_shapsummary_top20_{safe_name}.pdf"),
            bbox_inches="tight",
            dpi=300)
        plt.close()
        ...
        shap.summary_plot(
            shap_list[i],
            X_train_selected_df,
            max_display=len(clean_features),
            show=False
        )
        plt.savefig(
            os.path.join(plots_dir, f"{model}_shapsummary_allfeatures_{safe_name}.pdf"),
            bbox_inches="tight",
            dpi=300
        )
        plt.close()
    ...
    shap_df.to_csv(os.path.join(plots_dir, f"{model}_shapimportanceresults.csv"), index=False)

def get_base_model_name(model_name):
    if "lasso" in model_name.lower():
        return "Lasso Logistic Regression"
    elif "Logistic" in model_name or model_name.startswith("LR"):
...
def plot_roc_comparison(roc_data_dict, le, plots_dir, prefix=""):
    model_colors = {
        ...
        "Lasso Logistic Regression": "#c5b0d5"
    }
    ...
    plt.ylim(0, 1)
    plt.savefig(
        os.path.join(plots_dir, f"{prefix}allmodels_ROC_macro_comparison.pdf"),
        bbox_inches="tight",
        dpi=300,
        facecolor='white'
    )
    ...
    for i, class_name in enumerate(le.classes_):
        ...
        plt.grid(False)
        plt.savefig(
            os.path.join(plots_dir, f"{prefix}allmodels_ROC_compare_{class_name}.pdf"),
            bbox_inches="tight",
            dpi=300,
            facecolor='white'
        )
