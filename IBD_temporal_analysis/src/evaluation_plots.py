...
def display_confusion_matrix(model, y_test, y_pred, le, plots_dir):
    ...
    plt.savefig(
        os.path.join(plots_dir, f"pre&post2022_{model}_confusionmatrix.pdf"), bbox_inches="tight", dpi=300)
    # plt.close()
    plt.show()
    return cm
...
def sensitivity_specificity_per_class(model, cm, le, plots_dir):
    ...
    metrics_df.to_csv(os.path.join(plots_dir, f"pre&post2022_{model}_sensitivityspecificityresults.csv"), index=False)
    print("\nPer-class Sensitivity & Specificity:\n")
...
def display_roc_curves(model, y_test, y_prob, plots_dir, le):
    ...
    plt.savefig(
        os.path.join(plots_dir, f"pre&post2022_{model}_ROC_allclasses.pdf"),
        bbox_inches="tight",
        dpi=300
    )
    ...
    for i in range(len(classes_idx)):
        ...
        plt.savefig(
            os.path.join(plots_dir, f"pre&post2022_{model}_ROC_{le.classes_[i]}.pdf"),
            bbox_inches="tight",
            dpi=300
        )
    ...
    plt.savefig(
        os.path.join(plots_dir, f"pre&post2022_{model}_ROC_macro_only.pdf"),
        bbox_inches="tight",
        dpi=300
    )
...
def shap_plot(model, final_model, X_train_selected, clean_features, le, plots_dir):
    ...
    for i, class_name in enumerate(class_labels):
        ...
        plt.savefig(
            os.path.join(plots_dir, f"pre&post2022_{model}_shapsummary_{safe_name}.pdf"),
            bbox_inches="tight",
            dpi=300)
        plt.close()
    ...
    shap_df.to_csv(os.path.join(plots_dir, f"pre&post2022_{model}_shapimportanceresults.csv"), index=False)
...
def plot_roc_comparison(roc_data_dict, le, plots_dir, prefix=""):
    ...
    plt.savefig(
        os.path.join(plots_dir, f"pre&post2022_{prefix}allmodels_ROC_macro_comparison.pdf"),
        bbox_inches="tight",
        dpi=300,
        facecolor='white'
    )
    ...
    for i, class_name in enumerate(le.classes_):
        ...
        plt.savefig(
            os.path.join(plots_dir, f"pre&post2022_{prefix}allmodels_ROC_compare_{class_name}.pdf"),
            bbox_inches="tight",
            dpi=300,
            facecolor='white'
        )
